...
template<class T>
const T&
matrix_t<T>::at(const int i, const int j) const //nos devuelve el valor en una coordenada
{
  assert(i > 0 && i <= get_m());
  assert(j > 0 && j <= get_n());
  return v_[pos(i, j)];
...
template<class T>
const T&
matrix_t<T>::operator()(const int i, const int j) const // con esta sobrecarga del () podemos especificar la fila y columna del valor
{
  return at(i, j);
}
