// AUTOR: Jesus Carmelo Gonzalez Dominguez
// FECHA: 26/05/2020
...
{ 
  ...
  return (i >= 1 && i <= matrix_.get_m()) && (j >= 1 && j <= matrix_.get_n()) && (matrix_(i,j) != WALL_ID && !visited_(i,j));
}
...
maze_t::solve_(const int i, const int j)
{
  ...
  if (is_ok_(i-1,j)) //Norte
  {
    if (solve_(i-1,j))
    {
      matrix_(i-1,j) = PATH_ID;
      l_sol_.insert_head(new dll_node_t<char>('N'));
      return true;
    }        
  }
  if (is_ok_(i,j+1)) //Este
  {
    if (solve_(i,j+1))
    {
      matrix_(i,j+1) = PATH_ID;
      l_sol_.insert_head(new dll_node_t<char>('E'));
      return true;
    }        
  }    
  ...
  if (is_ok_(i+1,j)) //Sur
  {
    ...
    {
      matrix_(i+1,j) = PATH_ID;
      l_sol_.insert_head(new dll_node_t<char>('S'));
      return true;
    }        
  }
     
  if (is_ok_(i,j-1)) //Oeste
  {
    if (solve_(i,j-1))
    {
      matrix_(i,j-1) = PATH_ID;
      l_sol_.insert_head(new dll_node_t<char>('O'));
      return true;
    }        
  }            
   count_++; //contador del numero de retrocesos
...
//Parte de la fase III y de la modificacion: Para sacar el camino y el numero de retrocesos
void
maze_t::maze_path(void)
{
  if (l_sol_.empty())
  {
    cout << "No hay camino posible para el laberinto" << endl;
  }
  else
  {
    dll_node_t<char>* aux=l_sol_.get_head();
    cout << "Existe un camino con las siguientes coordenadas, partiendo desde A y ha retrocedido (backtrack): " << count_ << endl;
    if (aux)
    {
      cout << "A: ";
      while (aux)
