...
Regular_Ant::Regular_Ant(World &mundo, int x, int y, string direction) {
...
// Functions
/*
void Regular_Ant::Place_Ant(World &mesh, int x, int y, string dir) {
...
void Regular_Ant::Move_Up(Infinite_World &mesh) {
...
void Regular_Ant::Move_Left(Infinite_World &mesh) {
  mesh.At_Position( get_currentx() , get_currenty() ).set_direction(" ");
...
void Regular_Ant::Move_Right(Infinite_World &mesh) {
  mesh.At_Position( get_currentx() , get_currenty() ).set_direction(" ");
  int aux = get_currenty() + 1;
...
void Regular_Ant::Move_Down(Infinite_World &mesh) {
  mesh.At_Position( get_currentx() , get_currenty() ).set_direction(" ");
  int aux = get_currentx() + 1;
  if (aux >= mesh.get_row()) {
    set_resizenegative(false);
...
void Regular_Ant::Move_Up_Left(Infinite_World &mesh) {
  Move_Up(mesh);
  mesh.At_Position( get_currentx() , get_currenty() ).set_direction(" ");
  Move_Left(mesh);
...
void Regular_Ant::Move_Up_Right(Infinite_World &mesh) {
  Move_Up(mesh);
  mesh.At_Position( get_currentx() , get_currenty() ).set_direction(" ");
  Move_Right(mesh);
...
void Regular_Ant::Move_Down_Left(Infinite_World &mesh) {
  Move_Down(mesh);
  mesh.At_Position( get_currentx() , get_currenty() ).set_direction(" ");
  Move_Left(mesh);
...
void Regular_Ant::Move_Down_Right(Infinite_World &mesh) {
  Move_Down(mesh);
  mesh.At_Position( get_currentx() , get_currenty() ).set_direction(" ");
  Move_Right(mesh);
...
void Regular_Ant::Run_Antf(Finite_World &mesh) {
  if (mesh.At_Position( get_currentx() , get_currenty() ).get_state() == '0') {
    mesh.At_Position( get_currentx() , get_currenty() ).set_state('X');
    if (mesh.At_Position( get_currentx() , get_currenty() ).get_direction() == UP) {
      Move_Leftf(mesh);
...
void Regular_Ant::Move_Upf(Finite_World &mesh) { 
  mesh.At_Position( get_currentx() , get_currenty() ).set_direction(" ");
  int aux = get_currentx() - 1;
  if (aux < 0) {
    aux = mesh.get_row() - 1;
...
void Regular_Ant::Move_Leftf(Finite_World &mesh) {
  mesh.At_Position( get_currentx() , get_currenty() ).set_direction(" ");
  int aux = get_currenty() - 1;
  if (aux < 0) {
...
void Regular_Ant::Move_Rightf(Finite_World &mesh) {
  mesh.At_Position( get_currentx() , get_currenty() ).set_direction(" ");
  int aux = get_currenty() + 1;
  if (aux >= mesh.get_column()) {
    aux -= mesh.get_column() ;
...
void Regular_Ant::Move_Downf(Finite_World &mesh) {
  mesh.At_Position( get_currentx() , get_currenty() ).set_direction(" ");
  int aux = get_currentx() + 1;
  if (aux >= mesh.get_row()) {
    aux -= mesh.get_row();
