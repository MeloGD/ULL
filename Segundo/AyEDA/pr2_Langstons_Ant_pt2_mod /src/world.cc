...
void World::Resize_For_Move(int row, int column, World& mesh) {
  ...
  if (row < 0) {
    row = row * (-1);
    mesh.Resize_World(mesh.get_row() + row, mesh.get_column());
    for (int j = 0; j < auxiliar.get_size(); j++) {
      for (int k = 0; k < auxiliar[j].get_size(); k++) {
        mesh_[j + row][k] = auxiliar[j][k];
      }    
    }
  } else if (column < 0) {
    column = column * (-1);
    mesh.Resize_World(mesh.get_row(), mesh.get_column() + column);
    for (int j = 0; j < auxiliar.get_size(); j++) {
      for (int k = 0; k < auxiliar[j].get_size(); k++) {
        mesh_[j][k + column] = auxiliar[j][k];
      }
    }
  } else if (column > 0) {
    mesh.Resize_World(mesh.get_row(), mesh.get_column() + column);
