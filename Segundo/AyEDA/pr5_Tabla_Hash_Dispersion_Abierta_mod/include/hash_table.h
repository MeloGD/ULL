...
template<class Key>
void Hash_Table<Key>::Insert(Keys& clave) {
  //std::string y = x;
  int value = clave.get_value();
  std::string letters = clave.get_letters();
  //std::cout << "el valor es" << letters;
  
  int sum = 0;
  for (unsigned i = 0; i < letters[i]; i++) {
    sum = sum + x[i];
  }
  int position = sum + value;
  if (Add(position)) {
  dispersionfunction_->set_size(get_tablesize());
  datavector_[dispersionfunction_->operator()(position)].Set_Value(clave);
  } 
}
